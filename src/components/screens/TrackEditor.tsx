...
const OBJECT_FOOTPRINTS: Record<
  KitchenItemType,
  { w: number; d: number; shape: "rect" | "oval" }
> = {
  ...
  bowl: { w: 28, d: 28, shape: "oval" },
  cheese: { w: 28, d: 20, shape: "rect" },
  cheeseburger: { w: 24, d: 24, shape: "oval" },
  pizzaSlice: { w: 24, d: 28, shape: "rect" },
  croissant: { w: 28, d: 20, shape: "rect" },
  banana: { w: 8, d: 32, shape: "rect" },
  broccoli: { w: 24, d: 24, shape: "oval" },
  toast: { w: 20, d: 24, shape: "rect" },
  pretzel: { w: 24, d: 24, shape: "oval" },
};
  ...
  switch (action.type) {
    ...
    case "REVERSE":
      return {
        ...state,
        past: pushHistory(state),
        points: [...state.points].reverse() as [number, number][],
      };
    ...
    case "DELETE_HAZARD":
      return {
        ...state,
        hazards: state.hazards.filter((_, i) => i !== action.index),
      };
    ...
    case "ADD_OBJECT": {
      ...
      for (const existing of state.objects) {
        ...
        if (dist < newR + existR) {
          const top =
            (existing.y ?? 0) + OBJECT_HEIGHTS[existing.type] * existing.scale;
      ...
      return {
        ...
        selectedObjectIndex: state.objects.length,
      };
    }
    ...
    case "ADD_TUNNEL":
      return {
        ...
        tunnels: [
          ...state.tunnels,
          { tStart: action.tStart, tEnd: action.tEnd },
        ],
      };
    ...
    case "DELETE_TUNNEL":
      return {
        ...state,
        tunnels: state.tunnels.filter((_, i) => i !== action.index),
      };
    ...
    case "ADD_LIGHT":
      return {
        ...
        selectedLightIndex: state.lights.length,
      };
    ...
    case "DELETE_SPEED_STRIP":
      return {
        ...state,
        speedStrips: state.speedStrips.filter((_, i) => i !== action.index),
      };

    case "ADD_BOOST_TRACK":
      return {
        ...state,
        boostTracks: [...state.boostTracks, action.boostTrack],
        boostTrackStartT: null,
      };
    ...
    case "DELETE_BOOST_TRACK":
      return {
        ...state,
        boostTracks: state.boostTracks.filter((_, i) => i !== action.index),
      };
    ...
    case "ADD_RAIN_ZONE":
      return {
        ...state,
        rainZones: [...state.rainZones, action.rainZone],
        rainZoneStartT: null,
      };
    ...
    case "DELETE_RAIN_ZONE":
      return {
        ...state,
        rainZones: state.rainZones.filter((_, i) => i !== action.index),
      };
...
export function TrackEditor() {
  ...
  const hazardRotateRef = useRef<{ idx: number; lastAngle: number } | null>(
    null,
  );
  const lightMoveRef = useRef<{
    idx: number;
    offX: number;
    offZ: number;
  } | null>(null);
  ...
  const draw = useCallback(() => {
    ...
    if (points.length >= 2) {
      ...
      for (let hi = 0; hi < hazards.length; hi++) {
        ...
        if (
          hz.centerX !== undefined &&
          hz.centerZ !== undefined &&
          hz.radius !== undefined
        ) {
          const [cx2, cy2] = gameToCanvas(
            hz.centerX,
            hz.centerZ,
            originX,
            originY,
          ...
          if (isHazSel) {
            ...
            ctx.beginPath();
            ctx.moveTo(
              cx2 + Math.cos(rot) * hz.radius,
              cy2 + Math.sin(rot) * hz.radius,
            );
            ctx.lineTo(rhx, rhy);
            ...
            ctx.fillText(
              ...
              cx2,
              cy2 + hz.radius + 11 * invZoom,
            );
          }
        } else if (hz.tStart !== undefined && hz.tEnd !== undefined) {
          const curve2 = trackCurvePoints(
            points,
            pointRotations,
            loopClosed,
            20,
          ...
          for (let i = startI; i <= endI; i++) {
            ...
            const ny = dx / len;
            left2.push([
              curve2[ii][0] + nx * (lo + hw),
              curve2[ii][1] + ny * (lo + hw),
            ]);
            right2.push([
              curve2[ii][0] + nx * (lo - hw),
              curve2[ii][1] + ny * (lo - hw),
          ...
          for (let i = right2.length - 1; i >= 0; i--)
            ctx.lineTo(right2[i][0], right2[i][1]);
          ctx.closePath();
          ctx.fillStyle = HAZARD_COLORS[hz.type];
          ctx.fill();
        }
      ...
      for (let li = 0; li < lights.length; li++) {
        ...
        if (lt.type === "spot") {
          const targetCx =
            lt.targetX !== undefined
              ? gameToCanvas(lt.targetX, lt.targetZ!, originX, originY)[0]
              : lx;
          const targetCy =
      ...
      if (activeTool === "light" && hover) {
        const [r2, g2, b2] = [
          (activeLightColor >> 16) & 0xff,
          (activeLightColor >> 8) & 0xff,
          activeLightColor & 0xff,
        ];
      ...
      for (let i = 0; i < objects.length; i++) {
        ...
        const fp = OBJECT_FOOTPRINTS[obj.type] ?? {
          w: 12,
          d: 12,
          shape: "oval" as const,
        };
      ...
      for (let bi = 0; bi < boostTracks.length; bi++) {
        ...
        for (let ii = s; ii <= e2; ii++) {
          ...
          const ny = dx / len2;
          band.push([
            samples[ii][0] + nx * (laneCenter + laneW / 2),
            samples[ii][1] + ny * (laneCenter + laneW / 2),
          ]);
          band2.push([
            samples[ii][0] + nx * (laneCenter - laneW / 2),
            samples[ii][1] + ny * (laneCenter - laneW / 2),
        ...
        for (let ii = band2.length - 1; ii >= 0; ii--)
          ctx.lineTo(band2[ii][0], band2[ii][1]);
        ctx.closePath();
    ...
    if (showDirectionArrows && points.length >= 2) {
      const arrowCurve = trackCurvePoints(
        points,
        pointRotations,
        loopClosed,
      ...
      for (let i = 1; i < arrowCurve.length; i++) {
        ...
        if (accumulated >= arrowSpacing) {
          ...
          ctx.moveTo(mx, my);
          ctx.lineTo(
            mx - headLen * Math.cos(angle - headAngle),
            my - headLen * Math.sin(angle - headAngle),
          );
          ctx.moveTo(mx, my);
          ctx.lineTo(
            mx - headLen * Math.cos(angle + headAngle),
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      if (
        ...
        stateRef.current.selectedPoints.length > 0
      ) {
        ...
        if (selectedPoints.length === 1) {
          ...
          dispatch({
            type: "SET_POINT_ROTATION",
            index: idx,
            rotation: rots[idx] + delta,
          });
        } else {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === "[" || e.key === "]") {
        ...
        if (activeTool === "light" && selectedLightIndex !== -1) {
          ...
          if (
            ...
            const angle =
              Math.atan2(lt.targetZ - lt.z, lt.targetX - lt.x) + delta;
            dispatch({
              type: "SET_LIGHT_TARGET",
              index: selectedLightIndex,
              targetX: lt.x + Math.cos(angle) * dist,
              targetZ: lt.z + Math.sin(angle) * dist,
            });
          }
          return;
        }
        if (activeTool === "hazard" && selectedHazardIndex !== -1) {
          ...
          dispatch({
            type: "SET_HAZARD_ROTATION",
            index: selectedHazardIndex,
            rotation: (hz.rotation ?? 0) + delta,
          });
      ...
      if (e.key === "Delete" || e.key === "Backspace") {
        if (
          ...
          dispatch({
            type: "DELETE_OBJECT",
            index: stateRef.current.selectedObjectIndex,
          });
          return;
        }
        if (
          stateRef.current.activeTool === "hazard" &&
          stateRef.current.selectedHazardIndex !== -1
        ) {
          e.preventDefault();
          dispatch({
            type: "DELETE_HAZARD",
            index: stateRef.current.selectedHazardIndex,
          });
          return;
        }
        if (
          stateRef.current.activeTool === "light" &&
          stateRef.current.selectedLightIndex !== -1
        ) {
          e.preventDefault();
          dispatch({
            type: "DELETE_LIGHT",
            index: stateRef.current.selectedLightIndex,
          });
          return;
        }
      }
  ...
  const findNearestSegment = (
    pos: [number, number],
  ): { afterIndex: number; px: number; py: number } | null => {
    const pts = stateRef.current.points;
    ...
    for (let i = 0; i < n; i++) {
      ...
      let t =
        lenSq > 0 ? ((pos[0] - a[0]) * dx + (pos[1] - a[1]) * dy) / lenSq : 0;
  ...
  const hitHazardHandle = (
    pos: [number, number],
    idx: number,
  ): "edge" | "rotate" | "body" | null => {
    const canvas = canvasRef.current;
  ...
  const hitLightHandle = (
    pos: [number, number],
    idx: number,
  ): "body" | "target" | "distance" | null => {
    const canvas = canvasRef.current;
    if (!canvas) return null;
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (activeTool === "pen") {
      ...
      if (selectedObjectIndex !== -1) {
        ...
        const sfp = OBJECT_FOOTPRINTS[selObj.type] ?? {
          w: 12,
          d: 12,
          shape: "oval" as const,
        };
        ...
        for (const [lx, ly] of corners) {
          ...
          if (screenDist <= EDITOR_HIT.cornerInner) {
            ...
            cornerDragRef.current = {
              idx: selectedObjectIndex,
              startDist: distFromCenter,
      ...
      if (selectedHazardIndex !== -1) {
        ...
        if (hit === "rotate") {
          ...
          dispatch({ type: "PUSH_HISTORY" });
          hazardRotateRef.current = {
            idx: selectedHazardIndex,
            lastAngle: angle,
          };
          return;
        }
      ...
      if (samples.length >= 2) {
        ...
        if (stateRef.current.boostTrackStartT === null) {
          ...
          dispatch({
            type: "ADD_BOOST_TRACK",
            boostTrack: {
              tStart,
              tEnd,
              side: stateRef.current.activeBoostSide,
            },
          });
        }
      }
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (isDraggingRef.current) {
      ...
      if (rotateDragRef.current !== null) {
        ...
        const newRot =
          stateRef.current.objects[idx].rotation + (currentAngle - lastAngle);
      ...
      if (activeTool === "move" && dragHazardIndexRef.current !== -1) {
        ...
        dispatch({
          type: "MOVE_HAZARD",
          index: dragHazardIndexRef.current,
          centerX: Math.round(gx * 100) / 100,
          centerZ: Math.round(gz * 100) / 100,
        });
        return;
      ...
      if (hazardMoveRef.current !== null) {
        ...
        dispatch({
          type: "MOVE_HAZARD",
          index: idx,
          centerX: Math.round((gx + offX) * 100) / 100,
          centerZ: Math.round((gz + offZ) * 100) / 100,
        });
        return;
      }
      if (hazardEdgeRef.current !== null) {
        ...
        if (hz.centerX !== undefined && hz.centerZ !== undefined) {
          ...
          const newRadius = Math.max(
            5,
            Math.round(Math.hypot(pos[0] - cx, pos[1] - cy) * 10) / 10,
          );
          dispatch({
      ...
      if (activeTool === "object" && dragObjectIndexRef.current !== -1) {
        ...
        dispatch({
          type: "MOVE_OBJECT",
          index: dragObjectIndexRef.current,
          x: Math.round(gx * 100) / 100,
          z: Math.round(gz * 100) / 100,
        });
        return;
      ...
      if (lightMoveRef.current !== null) {
        ...
        dispatch({
          type: "MOVE_LIGHT",
          index: idx,
          x: Math.round((gx + offX) * 100) / 100,
          z: Math.round((gz + offZ) * 100) / 100,
        });
        return;
      }
      if (lightTargetRef.current !== null) {
        ...
        dispatch({
          type: "SET_LIGHT_TARGET",
          index: idx,
          targetX: Math.round(gx * 100) / 100,
          targetZ: Math.round(gz * 100) / 100,
        });
        return;
      }
      if (lightDistanceRef.current !== null) {
        ...
        const newDist = Math.max(
          20,
          Math.round(Math.hypot(pos[0] - lx, pos[1] - ly) * 10) / 10,
        );
      ...
      if (activeTool === "select" && selectRectStartRef.current) {
        dispatch({
          type: "SET_SELECTION_RECT",
          rect: {
            x1: selectRectStartRef.current[0],
            y1: selectRectStartRef.current[1],
            x2: pos[0],
            y2: pos[1],
          },
        });
  ...
  const handleMouseUp = (e: React.MouseEvent<HTMLCanvasElement>) => {
    if (isPanningRef.current) {
      ...
      if (canvasRef.current)
        canvasRef.current.style.cursor = isSpaceDownRef.current
          ? "grab"
          : "crosshair";
      return;
    }
  ...
  const handleContextMenu = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (activeTool === "boostTrack") {
      ...
      for (let bi = 0; bi < bts.length; bi++) {
        ...
        for (let ii = s; ii <= e2; ii++) {
          if (
            Math.hypot(pos[0] - samples[ii][0], pos[1] - samples[ii][1]) < hitR2
          ) {
    ...
    for (let ti = 0; ti < tunnels.length; ti++) {
      ...
      for (let ii = s; ii <= e2; ii++) {
        if (
          Math.hypot(pos[0] - samples[ii][0], pos[1] - samples[ii][1]) < hitR
        ) {
  ...
  const handleTest = () => {
    ...
    if (!loopClosed) {
      alert(
        "Close the loop first — click the first point (green ring) to close the track.",
      );
      return;
    }
    const originX = originRef.current?.x ?? canvas.width / 2;
  ...
  const handleSave = () => {
    ...
    const controlPoints = points.map(
      ([cx, cy]) =>
        canvasToGame(cx, cy, originX, originY).map(
  ...
  const handleImportJSON = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        const lights: PlacedLight[] = (data.lights ?? []).filter((l) =>
  ...
  const selectedObj =
    state.selectedObjectIndex !== -1
      ? state.objects[state.selectedObjectIndex]
      : null;
      ...
      <div className="editor-toolbar">
          ...
          <div className="editor-section">
            ...
            {selectedObj && (
              <div style={{ marginTop: 6 }}>
                <label className="editor-label">
                  {OBJECT_LABELS[selectedObj.type]} (selected)
                </label>
            ...
            {state.objects.length > 0 && (
              <span style={{ opacity: 0.6, fontSize: 9 }}>
                {state.objects.length} object
                {state.objects.length !== 1 ? "s" : ""} placed
              </span>
            )}
          </div>
        )}
                ...
                {state.tunnels.map((tn, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        dispatch({ type: "DELETE_TUNNEL", index: i })
                      }
                    >
                      ×
                    </button>
        ...
        {state.activeTool === "hazard" && (
          <div className="editor-section">
            ...
            >
              {(["juice", "milk", "oil", "butter"] as HazardType[]).map(
                (type) => (
                  <button
                    key={type}
                    ...
                    style={{
                      ...
                      background:
                        state.activeHazardType === type
                          ...
                          : undefined,
                      color:
                        state.activeHazardType === type ? "#fff" : undefined,
                    }}
                    onClick={() =>
                      dispatch({
                        type: "SET_ACTIVE_HAZARD_TYPE",
                        hazardType: type,
                      })
                    }
                  >
            ...
            <label className="editor-label" style={{ marginTop: 6 }}>
              Radius: {state.activeHazardRadius}
            </label>
            <input
              className="editor-slider"
              type="range"
              ...
              onChange={(e) =>
                dispatch({
                  type: "SET_ACTIVE_HAZARD_RADIUS",
                  radius: Number(e.target.value),
                })
              }
            />

            {state.selectedHazardIndex !== -1 &&
              state.hazards[state.selectedHazardIndex] &&
              (() => {
            ...
            {state.hazards.filter((h) => h.centerX !== undefined).length ===
              0 && (
              <span
                style={{
                  opacity: 0.5,
                  ...
                  >
                    <button
                      style={{
                        ...
                        textAlign: "left",
                        background:
                          i === state.selectedHazardIndex
                            ? HAZARD_COLORS[hz.type]
                            : "rgba(0,0,0,0.3)",
                        color: "#fff",
                        padding: "1px 4px",
                        borderRadius: 2,
                      ...
                      onClick={() =>
                        dispatch({ type: "SELECT_HAZARD", index: i })
                      }
                    >
                      {hz.type}
                      {hz.radius !== undefined
                    ...
                    <button
                      ...
                      onClick={() =>
                        dispatch({ type: "DELETE_HAZARD", index: i })
                      }
                    >
                      ×
                    </button>
        ...
        {state.activeTool === "light" && (
          <div className="editor-section">
            ...
            >
              {(["point", "spot"] as LightType[]).map((t) => (
                <button
                  ...
                  style={{ fontSize: 9, padding: "2px 4px" }}
                  onClick={() =>
                    dispatch({ type: "SET_ACTIVE_LIGHT_TYPE", lightType: t })
                  }
                >
                  {t}
                </button>
              ...
              {LIGHT_COLOR_PRESETS.map((p) => (
                <button
                  ...
                  style={{
                    ...
                    background:
                      state.activeLightColor === p.hex ? p.css : undefined,
                    color: state.activeLightColor === p.hex ? "#000" : "#fff",
                  }}
                  onClick={() =>
                    dispatch({ type: "SET_ACTIVE_LIGHT_COLOR", color: p.hex })
                  }
                >
                  {p.label}
                </button>
            ...
            <label className="editor-label">
              Intensity: {state.activeLightIntensity.toFixed(1)}
            </label>
            <input
              className="editor-slider"
              type="range"
              ...
              onChange={(e) =>
                dispatch({
                  type: "SET_ACTIVE_LIGHT_INTENSITY",
            ...
            <label className="editor-label">
              Distance: {state.activeLightDistance}
            </label>
            <input
              className="editor-slider"
              type="range"
              ...
              onChange={(e) =>
                dispatch({
                  type: "SET_ACTIVE_LIGHT_DISTANCE",
            ...
            {state.activeLightType === "spot" && (
              <>
                <label className="editor-label">
                  Angle: {Math.round((state.activeLightAngle * 180) / Math.PI)}°
                </label>
                <input
                  className="editor-slider"
                  type="range"
                ...
                <label className="editor-label">
                  Penumbra: {state.activeLightPenumbra.toFixed(2)}
                </label>
                <input
                  className="editor-slider"
                  type="range"
                  ...
                  onChange={(e) =>
                    dispatch({
                      type: "SET_ACTIVE_LIGHT_PENUMBRA",
              ...
              (() => {
                ...
                return (
                  <div style={{ marginTop: 6 }}>
                      ...
                      {LIGHT_COLOR_PRESETS.map((p) => (
                        <button
                          ...
                          style={{
                            ...
                            padding: "1px 4px",
                            border: `2px solid ${p.css}`,
                            background: lt.color === p.hex ? p.css : undefined,
                            color: lt.color === p.hex ? "#000" : "#fff",
                          }}
                          onClick={() =>
                            dispatch({
                    ...
                    >
                      {lt.type === "spot" ? "[/]=rotate aim · " : ""}Del=delete
            ...
            {state.lights.length > 0 && (
              ...
              >
                {state.lights.length} light
                {state.lights.length !== 1 ? "s" : ""} placed
              </span>
            )}
          </div>
        )}
        ...
        {state.activeTool === "speedStrip" && (
          <div className="editor-section">
            ...
            >
              Click track to place boost pad (1.5x speed) · right-click to
              delete
            </span>
            {state.speedStrips.length > 0 && (
              <>
                <label className="editor-label">
                  Speed Strips ({state.speedStrips.length})
                </label>
                {state.speedStrips.map((ss, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        dispatch({ type: "DELETE_SPEED_STRIP", index: i })
                      }
                    >
                      x
                    </button>
        ...
        {state.activeTool === "boostTrack" && (
          <div className="editor-section">
            ...
            >
              {(["left", "right"] as const).map((side) => (
                <button
                  ...
                  style={{ fontSize: 9, padding: "2px 4px" }}
                  onClick={() =>
                    dispatch({ type: "SET_ACTIVE_BOOST_SIDE", side })
                  }
                >
                  {side}
                </button>
            ...
            {state.boostTracks.length > 0 && (
              <>
                <label className="editor-label" style={{ marginTop: 6 }}>
                  Boost Tracks ({state.boostTracks.length})
                </label>
                {state.boostTracks.map((bt, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        dispatch({ type: "DELETE_BOOST_TRACK", index: i })
                      }
                    >
                      x
                    </button>
          ...
          <div className="editor-section">
            ...
            {state.selectedPoints.length === 0 && (
              <span style={{ opacity: 0.5, fontSize: 9, display: "block" }}>
                click point to select · drag to box-select · shift-click to
                toggle
              </span>
            )}
            {state.selectedPoints.length > 1 && (
              <span style={{ opacity: 0.6, fontSize: 9, display: "block" }}>
                {state.selectedPoints.length} points selected · scroll=rotate ·
                drag=move
              </span>
            )}
            {state.selectedPoints.length === 1 &&
        ...
        {state.activeTool === "rain" && (
          <div className="editor-section">
            ...
            {state.rainZones.length > 0 && (
              <>
                <label className="editor-label">
                  Rain Zones ({state.rainZones.length})
                </label>
                {state.rainZones.map((rz, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() =>
                        dispatch({ type: "DELETE_RAIN_ZONE", index: i })
                      }
                    >
                      x
                    </button>
        ...
        <div className="editor-label" style={{ marginTop: "auto" }}>
          ...
          {state.loopClosed && (
            <span style={{ color: "#5db345", marginLeft: 4 }}>✓</span>
          )}
          <br />
