...
/**
 * Cheerio surface — baked oat cereal look.
 * Warm golden base with toasted speckles and subtle grain.
 */
export function makeCheerioColorTexture(size = 256): THREE.CanvasTexture {
  const [canvas, ctx] = makeCanvas(size);
  const rng = seededRng(42);

  // Base warm golden gradient
  const grad = ctx.createRadialGradient(size / 2, size / 2, 0, size / 2, size / 2, size * 0.7);
  grad.addColorStop(0, '#e09428');
  grad.addColorStop(0.5, '#d4881e');
  grad.addColorStop(1, '#b87018');
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, size, size);

  // Toasted dark speckles
  for (let i = 0; i < 220; i++) {
    const x = rng() * size;
    const y = rng() * size;
    const r = 1 + rng() * 4.5;
    const alpha = 0.15 + rng() * 0.35;
    const shade = rng() > 0.5 ? '#6b3a08' : '#8b5010';
  ...
  // Light highlight speckles (puffed oat look)
...
/**
 * Cheerio roughness texture — mostly rough with slight sheen variation.
 */
export function makeCheerioRoughnessTexture(size = 256): THREE.CanvasTexture {
  const [canvas, ctx] = makeCanvas(size);
  const rng = seededRng(77);

  ctx.fillStyle = '#b0b0b0'; // base ~0.69 roughness
  ctx.fillRect(0, 0, size, size);

  const imgData = ctx.getImageData(0, 0, size, size);
  const d = imgData.data;
  for (let i = 0; i < d.length; i += 4) {
    const v = 160 + (rng() - 0.5) * 60;
...
/**
 * Broccoli color texture — dark forest green with floret-like bumpy spotting.
 */
export function makeBroccoliColorTexture(size = 256): THREE.CanvasTexture {
  const [canvas, ctx] = makeCanvas(size);
  ...
  // Lighter floret-bump circles scattered across surface
  for (let i = 0; i < 120; i++) {
    const x = rng() * size;
    const y = rng() * size;
    const r = 4 + rng() * 12;
    ...
    const color = isLight ? '#4caf50' : '#1b5e20';
    ctx.globalAlpha = alpha;
    ctx.fillStyle = color;
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    d[i]     = Math.max(0, Math.min(255, d[i]     + noise * 0.4));
    d[i + 1] = Math.max(0, Math.min(255, d[i + 1] + noise));
    d[i + 2] = Math.max(0, Math.min(255, d[i + 2] + noise * 0.4));
  }
  ctx.putImageData(imgData, 0, 0);
...
/**
 * Broccoli roughness — very rough, matte surface.
 */
export function makeBroccoliRoughnessTexture(size = 256): THREE.CanvasTexture {
  const [canvas, ctx] = makeCanvas(size);
  ...
  const d = imgData.data;
  for (let i = 0; i < d.length; i += 4) {
    const v = 200 + (rng() - 0.5) * 40; // high roughness ~0.8
    d[i] = d[i + 1] = d[i + 2] = Math.max(0, Math.min(255, v));
    d[i + 3] = 255;
  }
  ctx.putImageData(imgData, 0, 0);
...
/**
 * Broccoli stem color — pale yellow-green stalk.
 */
export function makeBroccoliStemTexture(size = 128): THREE.CanvasTexture {
  const [canvas, ctx] = makeCanvas(size);
  ...
  for (let i = 0; i < 30; i++) {
    ...
    ctx.strokeStyle = rng() > 0.5 ? '#4a8a20' : '#8aca50';
    ctx.lineWidth = 0.5 + rng() * 1.5;
    ctx.beginPath();
    ctx.moveTo(x + (rng() - 0.5) * 4, 0);
    ctx.lineTo(x + (rng() - 0.5) * 4, size);
    ctx.stroke();
  }
