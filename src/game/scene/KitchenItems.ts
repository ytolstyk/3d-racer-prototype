...
function makeMug(mats: Materials): THREE.Group {
  ...
  g.add(body);
  const handle = new THREE.Mesh(new THREE.TorusGeometry(1.8, 0.4, 6, 8, Math.PI), mats.ceramic);
...
function makeNapkin(mats: Materials): THREE.Group {
  const g = new THREE.Group();
...
export const OBJECT_HEIGHTS: Record<KitchenItemType, number> = {
  mug: 6, spoon: 0.3, plate: 1, fork: 0.3, napkin: 0.5, saltShaker: 8, glass: 9,
  butterDish: 3, donut: 2, breadLoaf: 5, salami: 6, cheeseWedge: 5, apple: 4,
  berryCluster: 3, notepad: 0.5, pen: 0.6, pencil: 0.6, stickyNote: 0.5,
  cauliflower: 6,
};
...
export class KitchenItems {
  ...
  constructor() {
    ...
    const makers = [
      makeMug, makeSpoon, makePlate, makeFork, makeNapkin, makeSaltShaker,
      makeGlass, makeButterDish, makeDonut, makeBreadLoaf, makeSalami,
      makeCheeseWedge, makeApple, makeBerryCluster, makeCauliflower,
      makeNotepad, makePen, makePencil, makeStickyNote,
    ];

    const clusters: [number, number][] = [
      [120, 100], [-120, 100], [120, -100], [-120, -100],
      [0, 120], [0, -120], [-60, 80], [60, 80],
    ];
